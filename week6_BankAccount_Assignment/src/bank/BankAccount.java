...
public class BankAccount {
    ...
    /* earnInterest - 이자 지급 (잔고가 있는 경우 한)
     * @param percentage - 이자율 (정수)
     * @return 지급 성공하면 true, 실패하면 false */
    public boolean earnInterest(int percentage) {
        boolean result = false;
        if (balance == 0) {
            JOptionPane.showMessageDialog(null, "잔고가 없어 이자를 지급할 수 없습니다.");
        }
        else {
